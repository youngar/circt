...
firrtl.circuit "Two" attributes {annotations = [
  ...
  firrtl.memmodule @MWrite_ext(in W0_addr: !firrtl.uint<4>, in W0_en: !firrtl.uint<1>, in W0_clk: !firrtl.clock, in W0_data: !firrtl.uint<42>) attributes {dataType = !firrtl.uint<42>, depth = 12 : ui64, extraPorts = [], maskBits = 1 : ui32, numReadPorts = 0 : ui32, numReadWritePorts = 0 : ui32, numWritePorts = 1 : ui32, readLatency = 1 : ui32, writeLatency = 1 : ui32}
  // CHECK: firrtl.module @Child(in %sram_0_user_input: !firrtl.uint<3>, out %sram_0_user_output: !firrtl.uint<4>)
  firrtl.module @Child() {
    %0:4 = firrtl.instance MWrite_ext  @MWrite_ext(in W0_addr: !firrtl.uint<4>, in W0_en: !firrtl.uint<1>, in W0_clk: !firrtl.clock, in W0_data: !firrtl.uint<42>)
  ...
  // CHECK: firrtl.module @Two(in %sram_0_user_input: !firrtl.uint<3>, out %sram_0_user_output: !firrtl.uint<4>, in %sram_1_user_input: !firrtl.uint<3>, out %sram_1_user_output: !firrtl.uint<4>)
  firrtl.module @Two() {
    firrtl.instance child0 @Child()
    firrtl.instance child1 @Child()
    // CHECK: %child0_sram_0_user_input, %child0_sram_0_user_output = firrtl.instance child0  @Child
    // CHECK: %child1_sram_0_user_input, %child1_sram_0_user_output = firrtl.instance child1  @Child
...
firrtl.circuit "TestHarness" attributes {annotations = [
  ...
  firrtl.memmodule @MWrite_ext(in W0_addr: !firrtl.uint<4>, in W0_en: !firrtl.uint<1>, in W0_clk: !firrtl.clock, in W0_data: !firrtl.uint<42>) attributes {dataType = !firrtl.uint<42>, depth = 12 : ui64, extraPorts = [], maskBits = 1 : ui32, numReadPorts = 0 : ui32, numReadWritePorts = 0 : ui32, numWritePorts = 1 : ui32, readLatency = 1 : ui32, writeLatency = 1 : ui32}
  // CHECK: firrtl.module @DUT(in %sram_0_user_input: !firrtl.uint<3>, out %sram_0_user_output: !firrtl.uint<4>)
  firrtl.module @DUT() attributes {annotations = [{class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}]} {
    %0:4 = firrtl.instance MWrite_ext  @MWrite_ext(in W0_addr: !firrtl.uint<4>, in W0_en: !firrtl.uint<1>, in W0_clk: !firrtl.clock, in W0_data: !firrtl.uint<42>)
  }
  firrtl.module @TestHarness() {
    firrtl.instance dut @DUT()
    // CHECK: %dut_sram_0_user_input, %dut_sram_0_user_output = firrtl.instance dut  @DUT(in sram_0_user_input: !firrtl.uint<3>, out sram_0_user_output: !firrtl.uint<4>)
