...
firrtl.module @ForceableRegResetToNode(in %clock: !firrtl.clock, in %dummy : !firrtl.uint<1>, out %foo: !firrtl.uint<1>, out %ref : !firrtl.rwprobe<uint<1>>) {
  ...
  %reg, %reg_f = firrtl.regreset %clock, %one_asyncreset, %dummy forceable  : !firrtl.asyncreset, !firrtl.uint<1>, !firrtl.uint<1>, !firrtl.rwprobe<uint<1>>
...
firrtl.module @RegResetInvalidResetValueType(in %c : !firrtl.clock, out %out : !firrtl.uint<2>) {
  ...
  %reg = firrtl.regreset %c, %c1_asyncreset, %c0_ui1  : !firrtl.asyncreset, !firrtl.uint<1>, !firrtl.uint<2>
...
firrtl.module @Verification(in %clock: !firrtl.clock, in %p: !firrtl.uint<1>, out %o : !firrtl.uint<1>) {
  ...
  // CHECK-NOT: firrtl.assert
  firrtl.assert %clock, %p, %p, "assert1" 
  // CHECK-NOT: firrtl.assume
  firrtl.assume %clock, %p, %p, "assume1" 
  // CHECK-NOT: firrtl.int.unclocked_assume
...
firrtl.module private @RWProbeUnused(in %in: !firrtl.uint<4>, in %clk: !firrtl.clock, out %out: !firrtl.uint) {
  ...
  %r, %r_ref = firrtl.reg interesting_name %clk forceable  : !firrtl.uint, !firrtl.rwprobe<uint>
...
firrtl.module @Whens(in %clock: !firrtl.clock, in %a: !firrtl.uint<1>, in %reset: !firrtl.uint<1>) {
  ...
  // CHECK-NEXT:   firrtl.printf %clock, %reset, "foo!"  
  // CHECK-NEXT: }
  firrtl.when %reset {
    firrtl.printf %clock, %reset, "foo!"  
  } else {
  }

  // CHECK-NEXT:  firrtl.printf %clock, %reset, "bar!"  
  firrtl.when %false {
  } else {
    firrtl.printf %clock, %reset, "bar!"  
  }

  // CHECK-NOT:  firrtl.when
  firrtl.when %true {
  } else {
    firrtl.printf %clock, %reset, "baz!"  
  }
}
...
firrtl.module @name_prop(in %clock: !firrtl.clock, in %next: !firrtl.uint<8>, out %out_b: !firrtl.uint<8>) {
  // CHECK: %reg = firrtl.reg %clock  : !firrtl.uint<8>
  %reg = firrtl.reg droppable_name %clock  : !firrtl.uint<8>
