...
firrtl.nla @NLA1 []
 // expected-error @+1 {{the instance path cannot be a single element}}
...
firrtl.circuit "fallBackName" {
  ...
  firrtl.module @Zebra(){
    %bundle = firrtl.wire {annotations = [#firrtl<"subAnno<fieldID = 2, {circt.nonlocal = @nla, class =\"test\" }>">]}: !firrtl.bundle<valid: uint<1>, ready: uint<1>, data: uint<64>>
  }
}
...
firrtl.circuit "Foo"   {
  // expected-error @+1 {{operation with symbol:#hw.innerNameRef<@Bar::@b> was not found in module:"Bar"}}
