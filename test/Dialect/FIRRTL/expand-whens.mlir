...
// CHECK-NEXT:   firrtl.printf %clock, %[[PRINT_ENABLE]], "CIRCT Rocks!" 
...
// CHECK-NEXT:   firrtl.stop %clock, %[[STOP_ENABLE]], 0
// CHECK-NEXT:   %[[ASSERT_ENABLE:.+]] = firrtl.and %p, %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.assert %clock, %p, %[[ASSERT_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
// CHECK-NEXT:   %[[ASSUME_ENABLE:.+]] = firrtl.and %p, %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.assume %clock, %p, %[[ASSUME_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
// CHECK-NEXT:   %[[COVER_ENABLE:.+]] = firrtl.and %p, %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.cover %clock, %p, %[[COVER_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
...
// CHECK-NEXT:   firrtl.printf %clock, %[[ELSE_PRINT_ENABLE]], "CIRCT Rocks!" 
...
// CHECK-NEXT:   firrtl.stop %clock, %[[ELSE_STOP_ENABLE]], 1
// CHECK-NEXT:   %[[ELSE_ASSERT_ENABLE:.+]] = firrtl.and %[[NOT_P]], %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.assert %clock, %p, %[[ELSE_ASSERT_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
// CHECK-NEXT:   %[[ELSE_ASSUME_ENABLE:.+]] = firrtl.and %[[NOT_P]], %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.assume %clock, %p, %[[ELSE_ASSUME_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
// CHECK-NEXT:   %[[ELSE_COVER_ENABLE:.+]] = firrtl.and %[[NOT_P]], %enable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   firrtl.cover %clock, %p, %[[ELSE_COVER_ENABLE]], ""  {eventControl = 0 : i32, isConcurrent = false}
...
firrtl.module @LayerUnderWhen(in %cond : !firrtl.uint<1>, in %clock : !firrtl.clock) {
  firrtl.when %cond {
    firrtl.layerblock @Layer {
      %c1_ui1 = firrtl.constant 1 : !firrtl.uint<1>
      firrtl.printf %clock, %c1_ui1, "Condition is true"
    }
  }
}
