...
firrtl.circuit "Top" {
  ...
  firrtl.module @Top(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset) attributes {
    ...
    // CHECK: [[ENUMCREATE:%[0-9]+]] = firrtl.enumcreate a(%c0_ui0) : !firrtl.enum<a>
    // CHECK: %reg_enum_0 = firrtl.regreset %clock, %reset, [[ENUMCREATE]] : !firrtl.asyncreset, !firrtl.enum<a>, !firrtl.enum<a>
    %reg_enum_0 = firrtl.reg %clock : !firrtl.enum<a>
    // CHECK: [[BITCAST:%[0-9]+]] = firrtl.bitcast %c0_ui1 : (!firrtl.uint<1>) -> !firrtl.enum<a = 1>
    // CHECK: %reg_enum_1 = firrtl.regreset %clock, %reset, [[BITCAST]] : !firrtl.asyncreset, !firrtl.enum<a = 1>, !firrtl.enum<a = 1>
    %reg_enum_1 = firrtl.reg %clock : !firrtl.enum<a = 1>
  }
}
...
firrtl.circuit "top" {
  ...
  firrtl.module private @test(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset, in %in: !firrtl.uint<8>, out %out: !firrtl.uint<8>) {
    ...
    %reg1 = firrtl.regreset %clock, %reset, %resetvalue : !firrtl.asyncreset, !firrtl.uint<8>, !firrtl.uint<8>
    firrtl.matchingconnect %reg1, %in : !firrtl.uint<8>
