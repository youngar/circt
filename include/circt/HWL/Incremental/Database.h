#ifndef CIRCT_HWL_INCREMENTAL_DATABASE_H
#define CIRCT_HWL_INCREMENTAL_DATABASE_H
...
public:
  ...
  explicit DatabaseHandle(D *database) : database(database) {}

  D *operator->() const { return database; }
  D &operator*() const { return *database; }
  D *getDatabase() const { return database; }
...
protected:
  Revision incRevision() { return this->getDatabase()->incRevision(); }
};
...
public:
  ...
  Input(Args &&...args) : InputBase(), value(std::forward<Args>(args)...) {
    // static_assert(IsDatabase<D>);
  }

  template <typename D, typename... Args>
  Input(DatabaseHandle<D> ctx, Args &&...args)
      : InputBase(ctx.getRevision()), value(std::forward<Args>(args)...) {
    // static_assert(IsDatabase<D>);
  }
...
template <typename D>
class QueryContext final : public DatabaseHandle<const D> {
  template <typename T>
  friend class Query;
  template <typename T>
...
protected:
  QueryContext(const D *database, DependencySet *dependencySet)
      : DatabaseHandle<const D>(database), dependencySet(dependencySet) {}
...
struct CachedQueryInfo : llvm::DenseMapInfo<CachedQuery<T> *> {
  ...
  static bool isEqual(const KeyTy<T> &key, CachedQuery<T> *entry) {
    if (entry == getTombstoneKey() || entry == getEmptyKey())
      return false;
