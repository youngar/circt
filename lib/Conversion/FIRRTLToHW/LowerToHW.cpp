...
struct CircuitLoweringState {
  ...
  std::atomic<bool> used_RANDOMIZE_MEM_INIT{false};
  std::atomic<bool> usedMemorySchema{false};
...
private:
  ...
  /// Cache of module symbols.  We need to test hierarchy-based properties to
  /// lower annotations.
  InstanceGraph *instanceGraph;
  ...
  for (auto &op : make_early_inc_range(circuitBody->getOperations())) {
    TypeSwitch<Operation *>(&op)
        ...
        .Case<FMemModuleOp>([&](auto memModule) {
          auto loweredMemModule =
              lowerMemModule(memModule, topLevelModule, state);
          if (!loweredMemModule)
            return signalPassFailure();
          state.oldToNewModuleMap[&op] = loweredMemModule;
        })
...
void FIRRTLModuleLowering::lowerFileHeader(CircuitOp op,
  ...
  // Emit the memory generator schema.
  if (state.usedMemorySchema) {
...
FIRRTLModuleLowering::lowerMemModule(FMemModuleOp oldModule,
                                     Block *topLevelModule,
                                     CircuitLoweringState &loweringState) {
  loweringState.used_RANDOMIZE_MEM_INIT = true;
  loweringState.usedMemorySchema = true;
  ...
  // Build the new hw.module op.
  auto builder = OpBuilder::atBlockEnd(topLevelModule);
  auto schema = FlatSymbolRefAttr::get(builder.getContext(), "FIRRTL_Memory");
  auto dict = oldModule->getAttrDictionary();

  SmallVector<NamedAttribute> generatorParams = {
      *dict.getNamed("depth"), *dict.getNamed("numReadPorts")};

  auto newModule = builder.create<hw::HWModuleGeneratedOp>(
      oldModule.getLoc(), schema, oldModule.getNameAttr(), ports);
...
FIRRTLModuleLowering::lowerModule(FModuleOp oldModule, Block *topLevelModule,
  ...
  annos.removeAnnotations([&](Annotation anno) {
    ...
    auto nla = dyn_cast<NonLocalAnchor>(
        loweringState.symbolTable.lookup(sym.getAttr()));
