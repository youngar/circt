...
struct Deduper {
  Deduper(MLIRContext *context, InstanceGraph &instanceGraph,
          SymbolTable &symbolTable, NLAMap &nlaMap)
      : context(context), instanceGraph(instanceGraph),
        symbolTable(symbolTable), nlaMap(nlaMap),
...
private:
  ...
  SmallVector<FlatSymbolRefAttr> createNLAs(OpBuilder &builder,
                                            FModuleLike toModule, AnnoTarget to,
    ...
    return createNLAsImpl(builder, toModule.moduleNameAttr(), to, fromModule,
                          namepath);
  }
  ...
  /// the NLAs.
  SmallVector<FlatSymbolRefAttr>
  createNLAs(OpBuilder &builder, FModuleLike toModule, StringAttr toModuleName,
    ...
    return createNLAsImpl(builder, toModuleName, to, fromModule, namepath);
  }
  ...
  /// for now.
  void makeAnnotationNonLocal(OpBuilder &builder,
                              SmallVector<FlatSymbolRefAttr> &nlas,
    ...
    if (nlas.empty())
      nlas = createNLAs(builder, toModule, toModuleName, to, fromModule);
  ...
  void mergeAnnotations(FModuleLike toModule, AnnoTarget to,
    ...
    for (auto anno : fromAnnos) {
      ...
      makeAnnotationNonLocal(builder, fromNLAs, toModule,
                             toModule.moduleNameAttr(), to, fromModule, from,
                             anno, newAnnotations);
    }
    ...
    for (auto pair : llvm::enumerate(toAnnos)) {
      ...
      makeAnnotationNonLocal(builder, toNLAs, toModule,
                             toModule.moduleNameAttr(), to, toModule, to, anno,
                             newAnnotations);
    }
...
class DedupPass : public DedupBase<DedupPass> {
  void runOnOperation() override {
    ...
    Deduper deduper(&getContext(), instanceGraph, symbolTable, nlaMap);
