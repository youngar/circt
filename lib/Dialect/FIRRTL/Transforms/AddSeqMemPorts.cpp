...
LogicalResult AddSeqMemPortsPass::processModule(FModuleOp module) {
  ...
  for (auto &op : llvm::make_early_inc_range(*module.getBody())) {
    ...
    if (auto inst = dyn_cast<InstanceOp>(op)) {
      ...
      // Connect each port up to the
      auto subPortIndex = 0;
      for (auto &[firstSubIndex, portInfo] : subExtraPorts) {
        // This is the index of the user port we are adding.
        auto userIndex = subPortIndex % userPorts.size();
        ...
        values.push_back(inst.getResult(firstSubIndex + subPortIndex));
        subPortIndex++;
      }
  ...
  if (userPorts.size() > 0) {
    ...
    if (auto dut = dyn_cast<FModuleOp>(*dutNode->getModule())) {
      ...
      for (auto *instRec : dutNode->uses()) {
        ...
        auto extraPortIndex = 0;
        for (auto &[firstResult, portInfo] : subExtraPorts) {
          if (portInfo.direction == Direction::Out)
            continue;
          auto value = inst.getResult(firstResult + extraPortIndex);
