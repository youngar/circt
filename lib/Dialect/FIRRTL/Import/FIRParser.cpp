...
private:
  ...
  ParseResult parseMatch(unsigned whenIndent);
...
ParseResult FIRStmtParser::parseEnumExp(Value &value) {
  ...
  if (parseToken(FIRToken::colon, "expected ':'") ||
      parseType(type, "expected type"))
    return failure();

  value = builder.create<FEnumCreateOp>(type, tag, input);
  return success();
}

ParseResult FIRStmtParser::parseMatch(unsigned indentLevel) {
  ...
  auto matchIndent = getIndentation();
  SmallVector<Attribute> tags;
  SmallVector<Block *> blocks;
  while (getToken().is(FIRToken::kw_case)) {
    ...
    auto *caseBlock = blocks.emplace_back(new Block());
    ...
    return subParser.parseSimpleStmtBlock(*caseIndent);
  }

  auto match = builder.create<MatchOp>(
      input, ArrayAttr::get(getContext(), tags), tags.size());

  // Copy the blocks in to the regions.
  for (auto [i, block] : llvm::enumerate(blocks))
    match.getRegion(i).push_back(block);
